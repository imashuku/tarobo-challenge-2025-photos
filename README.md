...
- 滋賀県・太郎坊宮の379段の石段を駆け上がった487名の挑戦者
- ゴール直前30mの表情を、マニュアルフォーカスで一日中追い続けた記録
- 1146枚のアーカイブから、ランダムに50枚をスライドショー表示
- Instagram風ダブルタップいいね、統計情報、人気ランキングを搭載
...
### 🆕 いいね機能（Supabase連携）
- **Instagram風UI**: ダブルタップで画面中央に大きなハート表示 🆕
- **サムネイル管理**: いいねフィルタ時にサムネイルから直接いいね解除可能 🆕
- **永続化**: Supabase PostgreSQLにいいね情報を保存
- **リアルタイム更新**: 統計情報がリアルタイムで反映（キャッシュ無効化） 🆕
- **通知機能**: いいね/いいね解除時に視覚的なフィードバック 🆕
...
### v2.1.0 (2025-10-23) 🆕
**UI/UX改善**
- ✨ Instagram風ダブルタップいいね機能実装
- ✨ いいねフィルタ時のサムネイルでいいね解除可能に
- ✨ 大きなハート通知表示（いいね/いいね解除）
- 🎨 スマホでの統計情報・Aboutウィンドウの表示を改善
- 🎨 統計情報とAboutを同じ幅に統一（280-320px）
- 📱 右側ボタンと干渉しないよう配置を最適化
- 📱 最大高さ制限とスクロール機能追加

**バグフィックス**
- 🐛 Supabaseのテストデータクリーンアップ機能
- 🐛 統計情報APIのキャッシュを無効化（リアルタイム更新）
- 🐛 ダブルタップ拡大エラーを修正（user-scalable=no追加）
- 🐛 dblclickイベント削除（フルスクリーン機能との競合解消）

**コンテンツ改善**
- 📝 Aboutテキストをより感動的な内容に改善
- 📝 具体的な数字を追加（487名の挑戦者、1146枚のアーカイブ）
- 📝 写らなかった方への配慮を追加
- 📝 いいね機能の使い方を明示

**技術的改善**
- 📦 統計パネルのスマホ用メディアクエリ最適化
- 📦 viewportに`user-scalable=no`追加
- 📦 パネルのスクロール設定追加（`overflow-y:auto`）

### v2.0.1 (2025-10-22)
**バグフィックス**
- 🐛 いいねボタンのエフェクト重複実行を修正
- 🐛 toggleLike関数の上書き時のコンテキスト保持を改善
- 🐛 いいね取り消し時の不要なアニメーションを削除

**技術的改善**
- 📦 関数の拡張方法を`.call(this)`で改善
- 📦 エフェクトの実行条件を最適化（いいね追加時のみ）
